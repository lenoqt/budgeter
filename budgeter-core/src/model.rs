...
pub struct Transaction {
    ...
    pub provider: String,
    /// Which budget member this transaction belongs to ("" = unassigned, or member name)
    #[serde(default)]
    pub member: String,
}
...
pub struct Mortgage {
    ...
    /// Fraction of monthly payment attributed to Member A (0.0–1.0) — input
...
pub struct Debt {
    ...
    pub interest_rate: f64,
    pub remaining_months: u32,
    /// Fraction of monthly payment attributed to Member A (0.0–1.0)
    #[serde(default = "default_half")]
    pub share_a: f64,
}
...
impl Loans {
    ...
    pub fn mortgage_a(&self) -> i64 {
        (self.mortgage.monthly_total as f64 * self.mortgage.share_a).round() as i64
    }
    pub fn mortgage_b(&self) -> i64 {
        self.mortgage.monthly_total - self.mortgage_a()
    }
    pub fn car_a(&self) -> i64 {
        (self.car.monthly_total as f64 * self.car.share_a).round() as i64
    }
    pub fn car_b(&self) -> i64 {
        self.car.monthly_total - self.car_a()
    }
    pub fn debts_a(&self) -> i64 {
        self.debts.iter()
            .map(|d| (d.monthly_payment as f64 * d.share_a).round() as i64)
            .sum()
    }
    pub fn debts_b(&self) -> i64 {
        self.debts.iter().map(|d| d.monthly_payment).sum::<i64>() - self.debts_a()
    }
    pub fn total_a(&self) -> i64 { self.mortgage_a() + self.car_a() + self.debts_a() }
    pub fn total_b(&self) -> i64 { self.mortgage_b() + self.car_b() + self.debts_b() }
}
...
impl Budget {
    ...
    pub fn balance_final(&self, s: IncomeScenario) -> i64 {
        // Use family.total() (the canonical stored total) and personal.total()
        // so this matches what summary() uses for balance_total.
        self.effective_income_total(s)
            - self.loan_total(s)
            - self.family_expenses.total()
            - self.personal_expenses.total()
    }
...
impl Budget {
    pub fn summary(&self, s: IncomeScenario) -> Summary {
        let income_total           = self.effective_income_total(s);
        let income_a               = self.effective_income_a(s);
        let income_b               = income_total - income_a;

        let loan_payment           = self.loan_total(s);
        let loan_a                 = self.loans.total_a();
        let loan_b                 = loan_payment - loan_a;

        // Use the canonical `total` field for the family row total so the
        // summary total matches what is shown on the Family tab.  For the
        // per-member split we also use the stored amount_a/amount_b, but we
        // clamp family_b so that family_a + family_b == family_total even if
        // the two halves were edited independently and drifted apart.
        let family_expense_total   = self.family_expenses.total();
        let family_expense_a       = self.family_expenses.total_a()
                                         .clamp(0, family_expense_total);
        let family_expense_b       = family_expense_total - family_expense_a;

        let personal_expense_total = self.personal_expenses.total();
        let personal_expense_a     = self.personal_expenses.total_a()
                                         .clamp(0, personal_expense_total);
        let personal_expense_b     = personal_expense_total - personal_expense_a;

        // Balance: compute total first, then derive B from total so that
        // balance_a + balance_b == balance_total is guaranteed.
        let balance_total = income_total - loan_payment - family_expense_total - personal_expense_total;
        let balance_a     = income_a - loan_a - family_expense_a - personal_expense_a;
        let balance_b     = balance_total - balance_a;
