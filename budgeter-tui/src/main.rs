...
fn handle_key(
    ...
    match app.popup.clone() {
        ...
        Popup::LoanMemberPicker => {
            return handle_loan_member_picker(app, code);
        }
        Popup::ImportMemberPicker => {
            return handle_import_member_picker(app, code);
        }
        Popup::MemberPicker { row, is_spending } => {
            return handle_tx_member_picker(app, db, code, row, is_spending);
        }
        Popup::None => {}
    }
    ...
    match app.active_tab {
        Tab::Import => {
            match code {
                ...
                // W — bulk-pick member to assign to all imported transactions
                KeyCode::Char('w') | KeyCode::Char('W') => {
                    app.open_import_member_picker();
                    return Ok(false);
                }
                // m — pick member for the currently selected import preview row
                KeyCode::Char('m') | KeyCode::Char('M') => {
                    if !app.import_preview.is_empty() {
                        app.open_tx_member_picker(app.import_selected, false);
                    }
                    return Ok(false);
                }
                _ => {}
        ...
        Tab::Spending => {
            match code {
                ...
                KeyCode::Char('m') | KeyCode::Char('M') if app.spending_drill => {
                    if !app.spending_drilled_transactions().is_empty() {
                        app.open_tx_member_picker(app.spending_tx_selected, true);
                    }
                    return Ok(false);
                }
                _ => {}
...
fn handle_tx_member_picker(
    ...
    let count = app.member_picker_names().len();
    match code {
        KeyCode::Esc => {
            app.popup = Popup::None;
        }
        KeyCode::Up | KeyCode::Char('k') => {
            if app.spending_member_cursor > 0 {
                app.spending_member_cursor -= 1;
            }
        }
        KeyCode::Down | KeyCode::Char('j') => {
            if app.spending_member_cursor + 1 < count {
                app.spending_member_cursor += 1;
            }
        }
        KeyCode::Enter => {
            app.confirm_tx_member(row, is_spending);
            if is_spending {
                if let Err(e) = db.save(&app.budget) {
                    app.set_status(format!("Save error: {e}"));
                } else {
                    app.set_status("Member updated and saved ✓");
                }
            }
        }
        _ => {}
...
fn handle_import_member_picker(app: &mut App, code: KeyCode) -> Result<bool> {
    ...
    match code {
        ...
        KeyCode::Up | KeyCode::Char('k') => {
            if app.import_member_cursor > 0 {
                app.import_member_cursor -= 1;
            }
        }
        KeyCode::Down | KeyCode::Char('j') => {
            if app.import_member_cursor + 1 < count {
                app.import_member_cursor += 1;
            }
        }
        KeyCode::Enter => {
            app.confirm_import_member();
        }
        _ => {}
    }
    Ok(false)
